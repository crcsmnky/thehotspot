...
def user(userid):
    ...
    places = {place['_id']: place for place in mongo.db.places2.find({'_id':{'$in':place_ids}})}
...
def place(placeid):
    place = mongo.db.places2.find_one_or_404({'_id': placeid})
    checkins_skip = int(request.args.get('skip', 0))
...
def allplaces():
    checkins_skip = int(request.args.get('skip', 0))
    places = mongo.db.places2.find({}).sort(
        'checkins', pymongo.DESCENDING).skip(checkins_skip).limit(10)
    count = mongo.db.places2.count()
...
def category(catid):
    ...
    places = mongo.db.places2.find({"categories": cat['name']}).sort(
