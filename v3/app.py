...
def tops():
    ...
    topplaces = mongo.db.places3.find({}).sort('checkins', pymongo.DESCENDING).limit(5)
...
def user(userid):
    ...
    places = {place['_id']: place for place in mongo.db.places3.find({'_id':{'$in':place_ids}})}
...
def place(placeid):
    place = mongo.db.places3.find_one_or_404({'_id': placeid})
    ...
    nearby = mongo.db.places3.find({"location": {"$near": place['location']}}).skip(1).limit(10)

    results = mongo.db.checkins.aggregate([
        {'$match': {'place_id': placeid}},
        {'$project': {'place_id': 1, 'dotw': {"$dayOfWeek": "$datetime"}}},
        {'$group': {'_id':{'place_id': "$place_id", 'dotw': "$dotw"}, 'count':{'$sum': 1}}}
    ])
    stats = {r['_id']['dotw']: r['count'] for r in results['result']}

    return render_template('place.html', place=place, users=users, checkins=checkins,
        nearby=nearby, places=nearby.clone(), stats=stats)


@app.route('/places/all', methods=['GET'])
def allplaces():
    checkins_skip = int(request.args.get('skip', 0))
    places = mongo.db.places3.find({}).sort(
        'checkins', pymongo.DESCENDING).skip(checkins_skip).limit(10)
    count = mongo.db.places3.count()
...
def categories():
    ...
    categories = mongo.db.categories.find().sort(
        "count", pymongo.DESCENDING).skip(cats_skip).limit(10)
    count = mongo.db.categories.count()

    return render_template('categories.html', categories=categories, count=count, skip=cats_skip)


@app.route('/categories/<int:catid>', methods=['GET'])
def category(catid):
    cat = mongo.db.categories.find_one_or_404({"_id": catid})
    places_skip = int(request.args.get('skip', 0))
    places = mongo.db.places3.find({"categories": cat['name']}).sort(
        'name', pymongo.ASCENDING).skip(places_skip).limit(10)

    return render_template('category.html', category=cat, places=places, mapped=places.clone(), count=cat['count'], skip=places_skip)
